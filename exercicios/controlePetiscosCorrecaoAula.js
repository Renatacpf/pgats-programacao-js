...
for (let petisco = 1; petisco <= quantidadePetiscosAleatoria; petisco++) {
    chance += 5
    const aleatoria = Math.random() * 100

    if (aleatoria <= chance) {
        console.log(`O Totó não quer mais petiscos!`);
        cheio = true
        break
    } else {
        console.log(`O Totó quer mais petiscos!`);
    }
    
    if (!cheio) {
        console.log(`O Totó comeu todos os petiscos mas segue com fome!`);
    }
}
